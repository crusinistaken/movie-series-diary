...
interface MediaItem {
  ...
  episode?: number;
  runtime?: string;
  rating?: number; // <--- YENİ: Puan alanı eklendi
}
...
const SavedCard = ({ item, isMovie, onRefresh }: { item: MediaItem, isMovie: boolean, onRefresh: () => void }) => {
  ...
  // Veri güncelleme (Durum veya Puan)
  const updateItem = async (updates: Partial<MediaItem>) => {
    ...
    try {
      await fetch("/api/media", {
        ...
        // Mevcut item verilerini yeni güncellemelerle birleştiriyoruz
  ...
  return (
    <div className="bg-gray-800 border border-gray-700 rounded-lg flex overflow-hidden hover:border-gray-500 transition shadow-lg group relative">
      ...
      <button 
        ...
        className="w-12 bg-gray-900/50 hover:bg-red-600/90 text-gray-500 hover:text-white flex items-center justify-center transition-all border-r border-gray-700"
      >
        <TrashIcon />
      </button>
      ...
      <div className="relative w-24 h-auto flex-shrink-0 bg-gray-700">
        <img 
          src={imgSrc} alt={item.title} className="w-full h-full object-cover"
          onError={() => setImgSrc("/images/placeholder.png")}
        />
        {/* Poster üzerine Puan Rozeti (Eğer puan varsa) */}
        {item.rating && item.rating > 0 && (
            <div className="absolute top-1 left-1 bg-yellow-500 text-black font-bold text-xs px-1.5 py-0.5 rounded shadow flex items-center gap-0.5">
                <span className="text-[10px]">★</span>{item.rating}
            </div>
        )}
      </div>
      ...
      <div className="flex-1 p-4 flex flex-col justify-between">
        <div>
          ...
          {/* Watching İstatistikleri */}
          {item.status === 'watching' && (
            <div className="text-blue-400 text-sm font-mono mt-1 flex items-center gap-2">
                {!isMovie ? ( <><span>📺</span> <span>S:{item.season} E:{item.episode}</span></> ) : ( <><span>⏳</span> <span>{item.runtime || "00:00"}</span></> )}
            </div>
          )}

          {/* --- YENİ BÖLÜM: SADECE COMPLETED İSE PUANLAMA GÖSTER --- */}
          {item.status === 'completed' && (
             <div className="mt-2 flex items-center gap-2 animate-fadeIn">
                ...
                <select 
                    ...
                    className="bg-gray-900 border border-gray-600 rounded px-2 py-1 text-sm text-white focus:border-yellow-500 focus:outline-none cursor-pointer"
                >
        ...
        <div className="mt-2 flex justify-end">
            {item.status === 'watching' && (
                <button onClick={() => updateItem({ status: 'completed' })} disabled={loading} className="flex items-center gap-2 bg-green-700 hover:bg-green-600 text-white px-3 py-1.5 rounded text-xs font-bold transition">
                    <CheckIcon /> Complete
                </button>
            )}
            {item.status === 'plan_to_watch' && (
                <button onClick={() => updateItem({ status: 'watching' })} disabled={loading} className="flex items-center gap-2 bg-blue-700 hover:bg-blue-600 text-white px-3 py-1.5 rounded text-xs font-bold transition">
                    <PlayIcon /> Start Watching
                </button>
            )}
            {item.status === 'completed' && (
                <span className="text-xs font-bold text-green-500 border border-green-500 px-2 py-1 rounded select-none">
                    COMPLETED
                </span>
            )}
        </div>
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        // Yeni kayıtta puan boş gider, sonradan completed sekmesinde eklenir
      };
...
export default function CategoryPage() {
  ...
  const fetchMyList = async () => {
    try { const res = await fetch("/api/media", { cache: "no-store" }); const json = await res.json(); if (json.data) setSavedItems(json.data); } catch (err) { console.error(err); }
  };
            ...
            {activeTab !== "add new" && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {filteredList.length > 0 ? ( filteredList.map((item) => ( <SavedCard key={item._id} item={item} isMovie={isMovie} onRefresh={fetchMyList} /> )) ) : (
                        <div className="col-span-2 text-center py-20 text-gray-500"><p className="text-xl mb-2">Empty list.</p></div>
                    )}
                </div>
            )}
        </div>
